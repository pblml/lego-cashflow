...
class PayPerUse():
    ...
    def __init__(self, acq_cost=288000, ref_quantity = 50, interest=0.025,  profit_pu=100, term=6, lower_bound=0.5):
    ...
    def calc(self, log_file, fixed_rate=4000):
        ...
        res_df["redemption_rate"] = [c*r if c>=self.lower_bound else self.lower_bound*r for c, r in zip(res_df["counter"], res_df["redemption_pu"])]
        res_df["remaining_debt"] = [self.acq_cost - sum(res_df["redemption_rate"][:i]) for i in range(len(res_df))]
        #!! FORMEL FÜR DIE ZINSBERECHNUNG FALSCH
        res_df["interest"] = res_df["remaining_debt"]*self.monthly_interest
        res_df["monthly_liability"] = res_df["redemption_rate"]+res_df["interest"]
        res_df["turnover"] = self.profit_pu*res_df["counter"]
        res_df["profits"] = res_df["turnover"]-res_df["redemption_rate"]
